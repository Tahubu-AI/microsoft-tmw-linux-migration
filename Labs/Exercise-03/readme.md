...
# Task 1: Review Azure Migrate PosgreSQL assessment
...
4. [] On the **Assessments** blade, select the **PaaS preferred (Recommended)** tab, then select the **PostgreSQL Instances to Azure Database for PostgreSQL** item.

    ![The PostgreSQL Instances to Azure Database for PostgreSQL item is highlighted in the list of workload to Azure service mappings.](./media/azure-migrate-project-assessment-paas-preferred.png)

5. [] On the **PostgreSQL Instances to Azure Database for PostgreSQL** blade, select the **localhost:5432** PostgreSQL instance in the list of discovered servers.
...
6. [] Select the **Readiness** tab and review any potential blockers or issues that need to be addressed before migration by selecting number specified for **Migration issues**.
...
7. [] Select the **Source properties** tab, and inspect the **Version** of the source database to ensure it is compatible with the migration service in Azure Database for PostgreSQL.
...
# Task 4: Validate the source database
